...
<?php
 ...
 $check_roll_no = mysqli_query($db, "SELECT * FROM student_register where roll_no = '".$roll_no."' ");
		$rowcount=mysqli_num_rows($check_roll_no);
		if($rowcount >0){
    ...
    <div class="row m50_0">
		...
		<div class="innercontsec">
          <form action="" method="post" enctype="multipart/form-data" onsubmit="return hello();">
            ...
            <input class="form-control modal_form" type="password" name="stu_pass" placeholder="Password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required />
